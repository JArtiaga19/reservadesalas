...
@ControllerAdvice // Anotación para indicar que esta clase manejará excepciones de forma global en la aplicación
public class GlobalExceptionHandler {

    @ExceptionHandler(IllegalArgumentException.class) // Anotación para indicar que este método manejará las excepciones de tipo IllegalArgumentException
    public ResponseEntity<?> handleIllegalArgument(IllegalArgumentException ex) { 
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ErrorResponse(ex.getMessage()));
    }  //Método que se ejecutará cuando se lance una IllegalArgumentException en cualquier parte de la aplicación
